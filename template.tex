...
%\hline
\begin{algorithmic}
  \STATE {\bf INPUT}: projection difference image 
  \STATE 1. $s\leftarrow$ localize the coordinates of the spherical marker center
  \STATE 2. $\hat{n}_1 \leftarrow$ estimate the initial flexible needle direction
  \STATE 3. $p_1, p_2, p_3, p_4 \leftarrow$ initialize coordinates of four landmark points from $s$ along $\hat{n}_1$
  \STATE 4. $L\leftarrow 3\Gamma$ initialize current length $L$
  \STATE 5. $i\leftarrow 4$ initialize number of landmark points
  ...
  \ENDWHILE
  \STATE 7. Adjust the coordinates of landmark point $p_i$ such that $length(C_i) = L_{marker,tip}$ 
  \STATE {\bf OUTPUT}: landmark points $p_i$ for $i=4,5,...$
  \STATE
  \STATE \hline 
\end{algorithmic}
...
Note that the projection difference image is signed, so pixels with large positive values correspond to coordinates in which the needle is present in the repeat scan but is absent in the baseline. Thus, the needle segment corresponds to a high intensity section of the difference image.
