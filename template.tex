...
This is achieved with lower dose compared to a full scan using sparse view angle sampling and without reconstructing the CT image of the repeat scan.
...
We performed registration and needle trajectory localization in seven abdomen phantom scans using two kinds of flexible needles, and obtained an average of 2.4 $\pm$ 0.9 mm tip localization error.
...
And finally, the patient's cumulative exposure to ionizing X-ray radiation may increase the risk of cancer \cite{mettler2000ct, chodick2007excess}, which is exacerbated in interventional CT due to the number of repeat scans acquired during the procedure.

Other imaging modalities used for interventional procedures include ultrasound, fluoroscopy and cone-beam CT.
Ultrasound is a cost effective imaging solution, often providing only 2D cross sections at limited resolution and signal-to-noise ratio (SNR) \cite{sheafor2000comparison}, though 3D/4D Ultrasound have become available in recent decades.
...
Flat-panel cone-beam CT is a relatively new technology increasingly used in interventional radiology. Though it suffered from longer scan times and increased radiation scatter, resulting in increased artifacts compared to multi-detector CT \cite{orth2008cbct}, recent advances have reduced scan times to 5-20 seconds, comparable to MDCT \cite{dynact}.
...
Yaniv et al. \cite{yaniv2010needle} describe a system in which embedded electromagnetic fiducials are used for needle tracking under CT guidance, with operator input required to identify the needle tip in a full CT scan taken in-situ. These methods rely on acquisition of full repeat scan and reconstruction of the image for each localization of the needle. This approach carries the associated cost in radiation dose for the patient. In the following section, we present a novel method for flexible needle localization in projection space using fractional scanning, without volume image reconstruction of the repeat scan, which promises to reduce the radiation dose associated with each needle localization.
\cite{medan2017reduced}

In a different clinical application, Cong et al.  \cite{cong2015quantitative} show the 3D structure of coronary arteries can be reconstructed from few views of X-ray angiography using forward projection of an evolving parametric 3D model into 2D views, updating the model based on the projection images.
...
\begin{enumerate}
    \item 
    {
    The 2D centers of the projected spherical marker are roughly localized in each of the $K$ projection difference images by correlating the difference projection image with a circular pattern. 
    }
    \item 
    {
    The 2D projected marker center in each projection difference image is refined by optimizing a cost function which takes into account the image gradients of the marker's surface:
    \begin{equation}
    ...
    \item
    {
    the 3D position of the marker center is calculated by solving an inverse problem using the known projection geometry of the CT and the localized marker centers, given as transformation matrices $P_j$. The set of projection equations $\{P_j s = c_j\}_{j=1}^K$ is solved to find the 3D marker center $s$.
    }
\end{enumerate}
...
The trajectory of the needle in 3D space is estimated by incrementally establishing a series of equidistant landmark points along the trajectory of the needle in 3D space $p_1, p_2, ..., p_i$. These landmark points are then fitted with a cubic B\'ezier curve $C_i$ tracing the curvilinear shape of the needle from the center of the spherical marker toward the last point $p_i$, where $i$ is the number of landmark points in the incremental process. The fit is achieved using a method by Khan \cite{khan2007approximation} to calculate a cubic B\'ezier curve in 3D space such that the squared-sum-distance of the landmark points from the curve is minimized. The sequence of landmark points is initialized with four co-linear, equidistant points starting from the marker center $p_1=s$, and with successive distance $\Gamma$ along an initial direction determined by optimization of a cost function detailed in the following paragraph. $\Gamma$ is a fixed parameter for segment length, and four is the minimal number of landmark points required to calculate a cubic B\'ezier curve. The algorithm is summarized as follows:
...
In a limited phantom study performed in cooperation with a leading CT manufacturer, an accuracy level comparable to 3D image space methods is shown in our experimental results.
